...
def detect_edges(image, kernel_size, low_ratio, high_ratio):
    ...
    my_edges = canny.hysteresis_thresholding(local_maxima, low_ratio, high_ratio)
...
async def edge_detection(file: UploadFile = File(...), kernel_size: int = Form(...), low_ratio: int = Form(...), high_ratio: int = Form(...)):
    # Đọc dữ liệu ảnh từ tệp tin
    image_data = await file.read()

    # Xử lý tìm cạnh
    edge_image = detect_edges(image_data, kernel_size, low_ratio, high_ratio)
